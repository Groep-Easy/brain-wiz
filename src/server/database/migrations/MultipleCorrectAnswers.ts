...
export class MultipleCorrectAnswers implements MigrationInterface {
  public name = 'MultipleCorrectAnswers'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."questions_theme_enum" AS ENUM('history', 'science', 'sport', 'culture', 'geography', 'technology', 'art', 'other')`
    )
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."rooms_selectedthemes_enum" AS ENUM('history', 'science', 'sport', 'culture', 'geography', 'technology', 'art', 'other')`
    )
    await queryRunner.query(
      `CREATE TYPE "public"."rooms_selectedlanguages_enum" AS ENUM('java', 'python', 'javascript', 'typescript', 'cpp', 'csharp', 'rust', 'go', 'other')`
    )
    await queryRunner.query(
      `CREATE TABLE "rooms" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "joinCode" character varying(16) NOT NULL, "qrCodePayload" character varying(512) NOT NULL, "status" "public"."rooms_status_enum" NOT NULL DEFAULT 'lobby', "hostSocketId" character varying(128), "selectedGameModes" "public"."rooms_selectedgamemodes_enum" array NOT NULL, "selectedThemes" "public"."rooms_selectedthemes_enum" array NOT NULL, "selectedLanguages" "public"."rooms_selectedlanguages_enum" array NOT NULL, "totalRounds" smallint NOT NULL DEFAULT '10', "defaultTimeLimitSeconds" smallint NOT NULL DEFAULT '20', "currentRoundIndex" smallint NOT NULL DEFAULT '0', "startedAt" TIMESTAMP WITH TIME ZONE, "finishedAt" TIMESTAMP WITH TIME ZONE, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_0368a2d7c215f2d0458a54933f2" PRIMARY KEY ("id"))`
    )
    await queryRunner.query(
      `CREATE INDEX "idx_rooms_join_code_active" ON "rooms"  ("joinCode") WHERE (status IN ('lobby', 'active'))`
    )
    await queryRunner.query(
      `CREATE TABLE "clients" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "roomId" uuid NOT NULL, "displayName" character varying(64) NOT NULL, "socketId" character varying(128), "isConnected" boolean NOT NULL DEFAULT false, "joinedAt" TIMESTAMP WITH TIME ZONE NOT NULL, "totalScore" integer NOT NULL DEFAULT '0', "finalRank" smallint, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_f1ab7cf3a5714dbc6bb4e1c28a4" PRIMARY KEY ("id"))`
    )
    ...
    await queryRunner.query(
      `CREATE TABLE "rounds" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "roomId" uuid NOT NULL, "roundIndex" smallint NOT NULL, "status" "public"."rounds_status_enum" NOT NULL DEFAULT 'pending', "contentType" "public"."rounds_contenttype_enum" NOT NULL, "timeLimitSeconds" smallint NOT NULL, "startedAt" TIMESTAMP WITH TIME ZONE, "finishedAt" TIMESTAMP WITH TIME ZONE, "questionId" uuid, "codingChallengeId" uuid, "puzzleId" uuid, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_9d254884a20817016e2f877c7e7" PRIMARY KEY ("id"))`
    )
    await queryRunner.query(
