...
1. Add the type string to `ROUNDS.TYPES` in `src/shared/constants/game-config.js` (git-master approval required).
2. Add server-side round logic in `src/server/game/rounds/<type>.js`.
3. Add host display screen in `src/host/screens/<type>/`.
4. Add client screen in `src/client/screens/<type>/`.
5. Add socket events if needed — add to `src/shared/events/socket-events.js` first.
